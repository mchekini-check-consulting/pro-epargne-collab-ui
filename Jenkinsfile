node("ci-node") {
    ...
    stage("Push Docker Image To Registry") {
        sh "sudo docker tag pro-epargne-collab-ui mchekini/pro-epargne-collab-ui:1.0"
        withCredentials([usernamePassword(credentialsId: 'mchekini', passwordVariable: 'password', usernameVariable: 'username')]) {
            sh "sudo docker login -u $username -p $password"
            sh "sudo docker push mchekini/pro-epargne-collab-ui:1.0"
            sh "sudo docker rmi mchekini/pro-epargne-collab-ui:1.0"
